...
from alpha_gen.models.alpha import Alpha, SimulationResult
...
class AlphaPolisher:
    ...
    def polish_alpha(
        ...
        try:
            ...
            if "alpha_details" in original_result and original_result["alpha_details"]:
            ...
            if "alpha_details" in polished_result and polished_result["alpha_details"]:
                ...
                if "is" in alpha_details and alpha_details["is"]:
                    ...
                    polished_alpha.metrics = Alpha.AlphaMetrics.from_api_format(metrics_data)
            ...
            if alpha.metrics:
                ...
                try:
                    ...
                    if "alpha_details" in result and result["alpha_details"]:
                        ...
                        # Update metrics
                        alpha.metrics = Alpha.AlphaMetrics.from_api_format(metrics_data)
    ...
    def _calculate_improvements(
        ...
        for key in ['sharpe', 'fitness', 'turnover', 'returns']:
            if key in original_metrics and key in polished_metrics:
                ...
                polished_value = polished_metrics.get(key, 0)
                
                if original_value != 0:
                    pct_change = ((polished_value - original_value) / abs(original_value)) * 100
                    improvements[f"{key}_change_pct"] = pct_change
                
                improvements[f"{key}_change"] = polished_value - original_value
                improvements[f"{key}_improved"] = (
                    (key == 'sharpe' and polished_value > original_value) or
                    (key == 'fitness' and polished_value > original_value) or
                    (key == 'returns' and polished_value > original_value) or
                    (key == 'turnover' and (
                        (original_value < 0.01 and polished_value >= 0.01) or
                        (original_value > 0.7 and polished_value <= 0.7) or
                        (0.01 <= original_value <= 0.7 and 0.01 <= polished_value <= 0.7)
                    ))
                )
        ...
        improvements["overall_improved"] = (
            ...
            improvements.get("fitness_improved", False) or
            improvements.get("turnover_improved", False)
        )
